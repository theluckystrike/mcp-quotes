...
test.skip("initialize and tools/list", async (t) => {
  const c = client();
  t.after(() => c.close());
  ...
  assert.equal(info.serverInfo.version, JSON.parse(readFileSync(join(REPO, "package.json"), "utf8")).version);
...
test.skip("create, revise, text-export and accept a quote; the invoice carries the same numbers", async (t) => {
...
test.skip("accept with no invoice store hands back invoice_create-ready items", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("decline, report and win rate", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("free tier: 5 open quotes and the text export, no PDF and no report", async (t) => {
  const c = client();
  t.after(() => c.close());
  await c.init();
  const ids = [];
...
test.skip("Pro renders an A4 PDF", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
