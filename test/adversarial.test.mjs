...
test.skip("bad arguments are refused by name, and nothing is stored", async (t) => {
...
test.skip("a total too large to represent exactly is refused before anything is written", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("two currencies on one quote are refused, not added together", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("accepting twice never issues a second invoice", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("an expired quote is refused until the price is re-confirmed", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("an expired quote does not eat a free open slot", async (t) => {
  const c = client();
  t.after(() => c.close());
  await c.init();
...
test.skip("an ambiguous client reference is refused with the candidates", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("stdout carries JSON-RPC only, even on the error paths", async (t) => {
  const c = client();
  t.after(() => c.close());
  await c.init();
  await c.tools();
...
test.skip("a VAT rate change between quote and acceptance never moves the agreed total", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
...
test.skip("quote_pdf to a directory is a refusal the caller can read, and the session survives", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
  const q = (await c.json("quote_create", { client: "A", items: [ITEM], currency: "EUR" })).created;
...
test.skip("validity_days at its boundaries: 0 refused, 3650 accepted, 3651 refused", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("a 1 MB notes field is refused by name, not stored", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("a 500-line line-item description and an oversized one", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
...
test.skip("oversized client name, email, address, VAT id and decline reason are all refused by name", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
  const cases = [
...
test.skip("accepting a quote continues the invoice server's own number series, not a fresh one", async (t) => {
  const c = client({ MCP_LICENSE_KEY: PRO });
  t.after(() => c.close());
  await c.init();
