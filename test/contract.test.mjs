...
// Contract suite for quotes. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/quotes/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-quotes-"));
...
test("every tool description is non-empty, single-paragraph, and within the hard ceiling", async (t) => {
  ...
  assert.equal(tools.length, 11, "tool count changed; update servers/quotes/SPEC.md");
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  await c.init();
  const r = await c.call("quote_create", {"client":"A","items":[{"description":"Work","quantity":1,"unit_price_minor":10000}],"currency":"EUR"});
...
test.skip("cap: no row-capped write tool on this server", { skip: "quotes has no row cap on a written file; its free limit is a count of OPEN quotes, asserted in test/smoke.test.mjs and test/adversarial.test.mjs" }, () => {});
